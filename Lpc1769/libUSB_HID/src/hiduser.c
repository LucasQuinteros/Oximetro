...
uint32_t HID_GetReport (void) {
  ...
  switch (SetupPacket.wValue.WB.H) {
    case HID_REPORT_INPUT:
      GetInReport(InReport, IN_REPORT_LEN);
      //EP0Buf[0] = InReport;
      for(i=0; i<IN_REPORT_LEN; i++) EP0Buf[i] = InReport[i];
      break;
  ...
  switch (SetupPacket.wValue.WB.H) {
    ...
    case HID_REPORT_OUTPUT:
      //OutReport[0] = EP0Buf[0];
    	for(i=0; i<OUT_REPORT_LEN; i++) OutReport[i] = EP0Buf[i];
      SetOutReport(OutReport, OUT_REPORT_LEN);
      break;
    case HID_REPORT_FEATURE:
      return (FALSE);          /* Not Supported */
  }
